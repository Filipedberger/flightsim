...
float Get_height(float x, float z, Model* model, TextureData* tex) {
    ...
    float height = (1 - u) * ((1 - v) * v1.y + v * v3.y) + u * ((1 - v) * v2.y + v * v4.y);
    return height;



	// KOLLA HUR TRIANGLARNA ÄR VÄNDA ##############
	if (isUpperTriangle) {
...
void init(void)
{
	...
	LoadTGATextureSimple("fft-terrain.tga", &tex1);
...
void display(void)
{
	...
	glUniformMatrix4fv(glGetUniformLocation(program, "mdlMatrix"), 1, GL_TRUE, total.m);
	DrawModel(octagon, program, "inPosition", "inNormal", NULL);
