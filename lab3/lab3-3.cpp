...
mat4 bodyPos, bladePos, balconyPos, groundPos, skyPos;
...
{	
	...
	textureProgram = loadShaders("lab3-3.vert", "lab3-3-texture.frag");
    glUseProgram(textureProgram);
	printError("init shader");
	

	glUniformMatrix4fv(glGetUniformLocation(textureProgram, "in_translation"), 1, GL_TRUE, in_translation);
	glUniformMatrix4fv(glGetUniformLocation(textureProgram, "in_scale"), 1, GL_TRUE, in_scale);
...
void display(void)
{
	...
	// Draw skybox
	glUseProgram(textureProgram);
	glUniformMatrix4fv(glGetUniformLocation(textureProgram, "viewMatrix"), 1, GL_TRUE, viewMatrix.m);
	glActiveTexture(GL_TEXTURE0);

	glDisable(GL_DEPTH_TEST);
	glDisable(GL_CULL_FACE);
	glBindTexture(GL_TEXTURE_2D, skyBoxtex);
	glUniformMatrix4fv(glGetUniformLocation(textureProgram, "mdlMatrix"), 1, GL_TRUE, skyPos.m);
	...
	glUniformMatrix4fv(glGetUniformLocation(program, "mdlMatrix"), 1, GL_TRUE, groundPos.m);
